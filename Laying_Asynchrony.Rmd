...
Bioedit <- Biodata %>% 
  ...
  mutate(Laying_async = as.numeric(PUESTA2 - PUESTA1)) %>% drop_na(Laying_async) %>% 
  filter(!Laying_async < 0) %>% mutate(HA = cut(Laying_async, c(-Inf, 2, 5, 8, 11, Inf), labels = c(0, 3, 6, 9, 12))) %>% 
