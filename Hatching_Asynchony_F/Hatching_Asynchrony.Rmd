...
path = "Conducta"
setwd("C:/Users/Conducta/Dropbox/PHD/Git/Hatching_Asynchrony/Hatching_Asynchony_F")
getwd()
```
...
Bioedit <- Biodata %>% 
  mutate(across(c(ECLOSION1, ECLOSION2, ECLOSION3), mdy)) %>% 
  mutate(Hatching_async = as.numeric(ECLOSION2 - ECLOSION1)) %>% 
  ...
  group_by(NIDO) %>% 
  mutate(DATEmin = min(c(ECLOSION1, ECLOSION2), na.rm = T)) %>% 
  mutate(DATEmax = max(c(ECLOSION1, ECLOSION2), na.rm = T)) %>% 
  filter(is.na(EDADESTIM2)) %>% # Remove nest where revision occures every 6 days
  as.data.frame()
...
ClimWin = slidingwin(xvar = list(ClimVar = Clim$SST), # Climatic variable
                     ...
                     baseline = glm(HA ~ PROPORTIONALRANK + AGEHEMB + I(AGEHEMB^2),
                                      ...
                                      data = Bio),
                     cinterval = "week",
                     range = c(7, 0), # max window used is 10   
                     type = "relative",
                     stat = c("min", "max", "mean"), 
...
png(file = str_glue("{ClimVar}plotdelta.png"), width=600*2, height=350*2)
plotdelta(dataset = Results)
dev.off()
png(file = str_glue("{ClimVar}plotweights.png"), width=600*2, height=350*2)
...
ClimWin <- slidingwin(xvar = list(ClimVar = Clim$SST), # Climatic variable
                      cdate = Clim$Date,
                      bdate = Bio$DATEmax,
                      baseline = glm(HA ~ PROPORTIONALRANK + AGEHEMB + I(AGEHEMB^2) + Area_Rain + I(Area_Rain^2),
                                      family = poisson,
                                      # nAGQ=0,
                                      data = Bio),
                      cinterval = "week",
...
ClimWin <- slidingwin(xvar = list(ClimVar = Clim$Area_Rain), # Climatic variable
                      cdate = Clim$Date,
                      bdate = Bio$DATEmax,
                      baseline = glm(HA ~ PROPORTIONALRANK + AGEHEMB + I(AGEHEMB^2) + SST,
                                     family = poisson,
                                     # nAGQ=0,
                                     data = Bio),
                      cinterval = "week",
...
<!-- real=ddply(DATAsurv, ~ Status + RainYear+ #AGEHEMB + I(AGEHEMB^2) + -->
...
<!-- ModGlmm = glmmTMB(HA ~ PROPORTIONALRANK + -->
<!--                     RefittedSST +  -->
<!--                     RefittedArea_Rain + I(RefittedArea_Rain^2) + -->
<!--                     AGEHEMB + I(AGEHEMB^2) + -->
<!--                     (1|ANILLOHEMB), -->
...
write_csv(CoefCLMMTable, "Climwin_HASize_PlasticityALL_fix.csv") # Climwin_HASize_PlasticityALL_fix.csv
...
write_csv(CoefCLMMTable, "Climwin_HASizeALL_B_W_fix.csv")
...
Data_HA <- Data %>%
  filter(!ANILLOHEMB == "SA") %>%
  mutate(across(c(PUESTA1, PUESTA2, PUESTA3), mdy)) %>%
  mutate(Laying_async = as.numeric(PUESTA2 - PUESTA1)) %>%
  filter(Laying_async >= 0 & Laying_async <= 30) %>% # remove outliers
  mutate(LA = cut(Laying_async, c(-Inf, 2, 5, 8, Inf), labels = c(0, 1, 2, 3))) %>%
  mutate(LA = as.numeric(as.character(LA))) %>%
  select(NIDO, WORKYEAR, HA, LA,
         ANILLOHEMB, AGEHEMB,
         PROPORTIONALRANK,
         RefittedSST, RefittedArea_Rain) %>%
  mutate(across(c(WORKYEAR), as.character)) %>%
...
write_csv(CoefCLMMTable, "Climwin_CLMM_LAcontrol_fixcoef.csv")
write_csv(RanCoef, "Climwin_CLMM_LAcontrol_rancoef.csv")
```