...
Bioedit <- Biodata %>% 
  select(ECLOSION1, ECLOSION2, ECLOSION3) %>% 
  mutate(across(c(ECLOSION1, ECLOSION2, ECLOSION3), ymd)) %>% 
  mutate(Hatching_async = as.numeric(ECLOSION2 - ECLOSION1)) %>% drop_na(Hatching_async) %>% 
  filter(!Hatching_async < 0) %>% mutate(HA = cut(Hatching_async, c(-Inf, 2, 5, 8, 11, Inf), labels = c(0, 3, 6, 9, 12))) %>% 
...
BioMini <- Bioedit %>% select(WORKYEAR, NIDO, DATEmin, ANILLOHEMB, HA)
```
...
```{r, message=FALSE}
BioData = read.csv("DATAWinHA.csv", header = T, sep = ",", stringsAsFactors = F) %>%names 
  select(DATEmin, NIDO, WORKYEAR, ANILLOHEMB, HA, 215:219) %>% 
  mutate(across(c(DATEmin), ymd))
```
